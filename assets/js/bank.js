...
(async () => {
  ...
  // 與「班級共同目標名稱」兩列已廢棄**（sync 不再讀它們）——**2026-09-20 老師已把那兩列刪除**。
  ...
  const storeSection = () => {
    const TIERS = [
      ...
      // ⑤ 刻意排在最後：實物是增強物四類中最末位，能用前面四層就不要動用這一層。
      // 品項一律以「老師身邊即可取得」為準（午餐剩的水果乳品、備用文具），不特意採購。
      ["⑤ 實物兌換・最後手段", "🎁 實物兌換（文具・食物）", "拿得到手的東西，但排在最後——前面四種都是花錢買不到的。"],
    ];
    ...
    const cards = tier => store.filter(i => (i.tier || "② 活動特權・消費型") === tier).map(i => `
      <div class="store-card ${!awardOnly(i) && i.stock <= 0 ? "soldout" : ""}">
        ...
        ${i.nextPrice ? `<div class="store-unlock">📢 ${App.esc(i.nextDate.slice(5).replace("-", "/"))} 起 ${Number(i.nextPrice)} 幣</div>` : ""}
        ${buyBtn(i)}
