  ...
  about.cadres = rosterRows
    ...
    .map(r => {
      ...
      return {
        ...
        authority: d?.["能管的事"] ?? "",
        bonus: d?.["額外獎金"] ?? "",
        // 幹部 8/27 才正式選出，在那之前名冊勾「職務預排」，班網在名字旁標「預排」。
        // 不標的話這份預排名單看起來就是定案，孩子與家長會當真。
        tentative: !!r["職務預排"],
...
const SCHEDULE_META = {
  periods: [
    ...
    { name: "第四節", time: "11:20-12:00" },   // 2026-09-14 老師更正：原寫 11:10，實際 11:20 上課（11:10–11:20 為下課）
  ...
  notes: "週三、週五為半天課，中午 12:40 放學\n體育課請穿運動服與運動鞋，帶水壺，可帶運動飲料，若太陽太大，可準備墨鏡活動\n星期一、四穿運動服。星期二、五穿制服。星期三穿便服\n星期二實體升旗會進行服儀檢查，要戴帽子、指甲修剪整齊",
...
// **已拍板的例外（2026-09-06・Phase 2-2、3-2）**：另外輸出 duties-seats.json／seating-seats.json／lunch-seats.json
...
async function syncClassDuties() {
  ...
  const { duties, lunch, seating, dutiesSeats, seatingSeats, lunchSeats, warnings } =
  ...
  await save("lunch-seats.json", lunchSeats);
}
